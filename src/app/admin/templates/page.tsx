...
const DEFAULT_PLACEHOLDERS: PlaceholderField[] = [
	{ key: "name", y: 1100, fontSize: 72, fontFamily: "Inter", color: "#333333", align: "center" },
	{
		key: "workshop_title",
		y: 1250,
		fontSize: 48,
	...
	{ key: "date", y: 1350, fontSize: 36, fontFamily: "Inter", color: "#888888", align: "center" },
];
...
export default function TemplatesPage() {
	...
	const [fields, setFields] = useState<PlaceholderField[]>([...DEFAULT_PLACEHOLDERS]);
	...
	const handleTestPreviewExisting = async () => {
		...
		setPreviewImg(null);
		try {
			const ext = editSelectedFile?.name.substring(editSelectedFile.name.lastIndexOf("."));
			const res = await apiSend<{ base64: string }>(
				"/api/admin/templates/preview",
				"POST",
				{
							...
							<div className="bg-gray-100 rounded-xl border-2 border-dashed border-gray-200 min-h-[300px] flex items-center justify-center overflow-hidden">
								{previewImg ? (
									...
									<div className="text-center text-gray-400 p-8">
										...
										<p className="text-sm">
											Upload an image and click <strong>Test Generate</strong> to
											see a preview with sample data
										</p>
									</div>
								)}
							</div>
						...
						<form onSubmit={handleEdit} className="flex flex-col h-full overflow-hidden">
							<div className="bg-linear-to-r from-te-orange to-te-red px-6 py-4 shrink-0">
								...
								<p className="text-white/80 text-xs mt-0.5">
									{editingTemplate.name} — {editingTemplate.width}×
								...
								<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
									<div className="space-y-4">
										...
										<div>
											...
											<PlaceholderEditor fields={editFields} onChange={setEditFields} />
										</div>
									</div>
									<div>
										<div className="flex items-center justify-between mb-2">
