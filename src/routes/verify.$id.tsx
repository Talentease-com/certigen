...
import { verifyCertificate } from "#/server/functions/certificates";
...
export const Route = createFileRoute("/verify/$id")({
  head: ({ loaderData }) => {
    ...
    if (!cert) {
      return {
        meta: [{ title: "Certificate Not Found — Certigen" }],
      };
    }
    return {
      meta: [
        ...
        {
          name: "description",
          content: `Verified certificate of completion for ${cert.name} — ${cert.workshopTitle}`,
        },
        { property: "og:title", content: `Certificate — ${cert.name}` },
        {
          property: "og:description",
          content: `${cert.name} completed "${cert.workshopTitle}" on ${cert.workshopDate}. Issued by Talentease.`,
        },
      ],
    };
  },
  loader: async ({ params }) => {
    const cert = await verifyCertificate({ data: { id: params.id } });
    return { cert };
  },
