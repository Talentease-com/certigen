...
function TemplatesPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const t = await listTemplates({ data: { token: identity.token } });
      setTemplatesList(t);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!identity?.token || !fileBase64 || !selectedFile) return;
    setSubmitting(true);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deleteTemplate({ data: { token: identity.token, id } });
      await loadData();
    } catch (err) {
                ...
                {previewImg ? (
                  ...
                  <div className="text-center text-gray-400 p-8">
                    ...
                    <p className="text-sm">
                      ...
                      <strong>Test Generate</strong> to see a preview with
                      sample data
                    </p>
                  </div>
                )}
